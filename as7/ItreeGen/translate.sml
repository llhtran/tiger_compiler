...
(* ------------------------- ARRAY / RECORD CREATION ------------------------ *)
  val ansStringCreated = ref false
  val ansString = ref (Ex (T.CONST 0))
  fun arrayItree(sizeTransExp, initTransExp) =
    let
      val size = unEx sizeTransExp
      val init = unEx initTransExp
      val r = Te.newtemp()
      val norm = Te.newlabel() and exit = Te.newlabel() and err = Te.newlabel()
      val sizeTemp = Te.newtemp()
    in
      (* Key for array bounds check: Create an extra slot for the array size.
         The array size goes in the first slot. *)
      if !ansStringCreated = false then (
        ansString := strItree("Negative array size\n");
        ansStringCreated := true
      ) else ();
      Ex(T.ESEQ(seq[T.MOVE(T.TEMP sizeTemp, size),
                    T.CJUMP(T.TEST(T.LT, T.TEMP sizeTemp, T.CONST 0), err, norm),
                    T.LABEL(norm),
                    T.MOVE(T.TEMP r, F.externalCall("initArray",
                        [T.BINOP(T.PLUS, size, T.CONST 1), init])),
                    T.MOVE(F.contentsOf(T.TEMP r), size),
                    T.JUMP(T.NAME exit, [exit]),
                    T.LABEL(err),
                    T.EXP(T.CALL(T.NAME(Te.namedlabel "print"), [unEx (!ansString)])),
...
(* ---------------------------- I-TREE ACCESSORS ---------------------------- *)
  ...
  fun arrayElt (vTransExp, ssTransExp) =
    ...
    in
      ...
      Ex(T.ESEQ(seq[T.MOVE(T.TEMP ret, sum(vexp, T.CONST 4)),
                    T.MOVE(T.TEMP ssTemp, ssexp),
                    T.CJUMP(T.TEST(T.LT, T.TEMP ssTemp, F.contentsOf(vexp)), norm1, err),
                    T.LABEL(norm1),
                    T.CJUMP(T.TEST(T.GE, T.TEMP ssTemp, T.CONST 0), norm2, err),
                    T.LABEL(norm2),
                    T.MOVE(T.TEMP ret, sum(vexp, sizeOfNWords(sum(T.TEMP ssTemp, T.CONST 1)))),
                    T.JUMP(T.NAME exit, [exit]),
                    T.LABEL(err),
  ...
  val rnStringCreated = ref false
  val rnString = ref (Ex (T.CONST 0))

  fun recordField (vTransExp, fieldNum) =
    let
      val vexp = unEx(vTransExp)
      val ret = Te.newtemp()
      val exit = Te.newlabel() and err = Te.newlabel() and norm = Te.newlabel()
    in
      if !rnStringCreated = false then (
        rnString := strItree("Dereferencing nil\n");
        rnStringCreated := true
      ) else ();
      (* Add wordSize * fieldNum to memory address containing record's first
         field. *)
      Ex(T.ESEQ(seq[T.CJUMP(T.TEST(T.EQ, vexp, T.CONST 0), err, norm),
                    T.LABEL(norm),
                    T.MOVE(T.TEMP ret, sum(vexp, sizeOfNWords(T.CONST fieldNum))),
                    T.JUMP(T.NAME exit, [exit]),
                    T.LABEL(err),
                    T.EXP(T.CALL(T.NAME(Te.namedlabel "print"), [unEx (!rnString)])),
                    T.EXP(T.CALL(T.NAME(Te.namedlabel "exit"), [])),
                    T.LABEL(exit)],
                F.contentsOf(T.TEMP ret)))
